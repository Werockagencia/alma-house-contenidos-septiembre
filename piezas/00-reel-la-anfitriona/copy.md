# 00 · Reel — La anfitriona

**Formato:** Reel 9:16 · 35 s · video final editado en Premiere (`Contenidos/03.mp4`) + portada diseñada
**Publicar:** viernes 25 de septiembre · 7:00 p. m. — **abre el capítulo III**
**Rol en la narrativa:** es el prólogo. Antes de mostrar los códigos de la casa, aparece quien la abre. Es la primera vez que la marca tiene cara y que se ve el local real (el wordmark en la pared, la barra de color, el lounge de pedicure, las tres estaciones). Resuelve el "respaldo profesional" que pide el ADN (2.2): la gente le compra a personas.
**Perfil al que le habla:** los tres. Es el contenido de confianza.

**Archivo a subir:** `Contenidos/03.mp4` (original 1080×1920). La versión `video/alma-house-la-anfitriona-web.mp4` es solo para la página de revisión.
**Portada:** `export/00-reel-portada.png` (subir como portada personalizada del reel).

## Caption

Toda casa tiene quien te abra la puerta.

Esta es la nuestra. Soñamos Alma House como el lugar al que siempre quisimos llegar: sin afán, sin ruido, con un café bien hecho y una hora que nadie te acelera.

Ya está abierta. Y te estamos esperando.

Alma House · Nails Bar
CC Montaña Plaza, local 7, piso 2 (Km 2 vía Tabio), Cajicá
Reserva tu hora por WhatsApp: 311 566 2051

## Hashtags

#AlmaHouse #Cajicá #NailBarCajicá #EmprendimientoFemenino #DetrásDeLaMarca #UnaHoraParaTi

## Texto alternativo (portada)

La fundadora de Alma House sonríe, sentada en una silla verde salvia bajo el logo de la marca y una barra de esmaltes. "Toda casa tiene quien te abra la puerta."

## Notas

- **Ajustar antes de publicar:** incluir el nombre de la fundadora en la segunda línea ("Soy ___, y soñé Alma House…") y, si quiere, una frase textual suya del video. El video no tiene subtítulos ni transcripción en el proyecto, así que el caption no la cita.
- Si el video no trae subtítulos quemados, activar los subtítulos automáticos de Instagram (mucha gente ve reels sin sonido).
- Etiquetar a la fundadora (su cuenta personal) en el reel para sumar alcance.
